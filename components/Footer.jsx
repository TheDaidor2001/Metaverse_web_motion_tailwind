    ...
    <div className={`${styles.innerWidth} mx-auto flex flex-col gap-8`}>
      ...
      <div className="flex flex-col">
        ...
        <footer className="flex items-center justify-between flex-wrap gap-4">
          ...
          <p className="font-normal text-[14px] text-white opacity-50">Copyright © {new Date().getFullYear()} Metaversus. All rights reserved.</p>
          <div className="flex gap-4">
            {socials.map((social) => (
              <img key={social.name} src={social.url} alt={social.name} className="w-[24px] object-contain cursor-pointer" />
            ))}
          </div>
        </footer>
      </div>
